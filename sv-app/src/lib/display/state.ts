...
import type { DisplaySampleData, DisplayLegendRowSample } from '$lib/display/types';
...
function withLegendCounts(rows: DisplayLegendRowSample[], counts: SessionCounts, total: number): DisplayLegendRowSample[] {
	return rows.map((row) => {
...
export const displayWaitingVisible = derived(displayState, ($state) => $state.footer.participantCount === 0);
